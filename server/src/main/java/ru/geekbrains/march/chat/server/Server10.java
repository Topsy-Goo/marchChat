...
public class Server10
{
    private static final String
            ...
            SESSION_START = "\nНачало сессии.",
            UNABLE_TOCREATE_HANDLER = "\nНе удалось создать ClientHandler10.",
...
// Авторизуем клиента по логину и паролю.
    public synchronized String syncValidateOnLogin (String login, String password, ClientHandler10 client)
    {
...
//Клиент запросил смену имени.
    public synchronized String syncChangeNickname (ClientHandler10 client, String newnickname)
    {
        String result = null;
        if (client != null)
...
//Удаляем клиента из списка подключенных клиентов.
    public synchronized void syncClientLogout (ClientHandler10 client)
    {
...
//Рассылаем указанное сообщение всем клиентам из нашего списка подключенных клиентов.
    public synchronized boolean syncBroadcastMessage (String msg, ClientHandler10 from)
    {
        ...
        for (Map.Entry<String, ClientHandler10> entry : map.entrySet())
        {
            ClientHandler10 client = entry.getValue();
...
//Пересылаем указанное сообщение автору и указанному клиенту.
    public synchronized boolean syncSendPrivateMessage (String nameTo, String message, ClientHandler10 clientFrom)
    {
            ...
            map != null)
        {
            ...
            for (Map.Entry<String, ClientHandler10> entry : map.entrySet())
            {
                ClientHandler10 clientTo = entry.getValue();
