...
public class Server8
{
    private static final String
            ...
            UNABLE_TOCREATE_HANDLER = "\nНе удалось создать ClientHandler8."
            ;
    ...
    private int port = 0;
    private final Map<String, Server8.UserEntry> map;
    ...
    private class UserEntry
    {
        ...
        ClientHandler8 client;

        UserEntry (String lgn, String psw, String nm, ClientHandler8 cl)
        {
        ...
        String validate (String lgn, String psw, ClientHandler8 cl)
        {
...
//Заполняем Map тестовыми значениями.
    ...
    {   //      name                   login, pass, name
        map.put("u1111", new Server8.UserEntry("1", "11", "u1111", null));
        map.put("u2222", new Server8.UserEntry("2", "22", "u2222", null));
        map.put("u3333", new Server8.UserEntry("3", "33", "u3333", null));
        map.put("u4444", new Server8.UserEntry("4", "44", "u4444", null));
    }// tmpFillUsersMap ()
...
// Подготовка к «отключению» сервра.
    private void serverGettingDown ()
    {
        ...
        {
            for (Map.Entry<String, Server8.UserEntry> entry : map.entrySet())
...
// выборочно отключать клиентов (посылать приватное сообщение CMD_EXIT; клиенты лишены такой возможности).
    ...
    public synchronized String syncValidateOnLogin (String login, String password, ClientHandler8 client)
    {
        ...
        if (login != null  &&  password != null  &&  map != null)
        {
            for (Map.Entry<String, Server8.UserEntry> entry : map.entrySet())
            {
                Server8.UserEntry user = entry.getValue();
    ...
    public synchronized boolean syncChangeNickname (String oldname, String newname)
    {
        ...
        Server8.UserEntry user = (map == null) ? null : map.get(oldname);
...
//Помечаем клиента, как покинувшего чат (просто в соответствующем UserEntry сбрасываем поле client в null).
    public synchronized void syncRemoveClient (ClientHandler8 client)
    {
        Server8.UserEntry user;
...
//Рассылаем указанное сообщение всем клиентам из нашего списка подключенных клиентов.
    public synchronized boolean syncBroadcastMessage (String msg, ClientHandler8 from)
    {
...
//Пересылаем указанное сообщение автору и указанному клиенту. Сообщение получаем в формате: «адресат сообщение».
    public synchronized boolean syncSendPrivateMessage (String nameTo, String message, ClientHandler8 clientFrom)
    {
            ...
            map != null)
        {
            ...
            for (Map.Entry<String, Server8.UserEntry> entry : map.entrySet())
            {
                Server8.UserEntry userTo = entry.getValue();
