...
import static ru.geekbrains.march.chat.client.Main11.WNDTITLE_APPNAME;
import static ru.geekbrains.march.chat.server.ClientHandler11.THREAD_SLEEPINTERVAL_250;
...
public class Controller11 implements Initializable
{
    ...
    private Thread threadIntputStream,
                   ...
                   threadMainLoop
                   ;
    private MessageStenographer11<ChatMessage> stenographer;
 ...
 */
        ...
        while (!chatGettingClosed)
        {
            ...
            {
                Platform.runLater(()->{
                    queuePoll(); //chatGettingClosed = !queuePoll(); < здесь оказывается нельзя делать так (вызов
        ...
        // потока JavaFX, и используемый здесь вызов «Platform.runLater» единственный на всё приложение.)
...
// «Вход/Выход» обрабатывались беспорядочно.
    @FXML public void onactionLoginout (ActionEvent actionEvent)  {  closeSession (PROMPT_YOU_ARE_LOGED_OFF);  }
...
// Сейчас имя файла состоит из логина пользователя и расширения chat.
    ...
    {
        stenographer = new MessageStenographer11<> (login +".chat");
        List<ChatMessage> cmlist = stenographer.getData();
        txtareaMessages.clear();  //< очищаем окно чата (т.к. юзер может перезайти и под другой учёткой
